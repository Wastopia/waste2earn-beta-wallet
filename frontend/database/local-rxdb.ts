...
export class LocalRxdbDatabase extends IWalletDatabase {
  ...
  async getKYCDetails(userId: string): Promise<KYCDetails | null> {
    try {
      ...
      logger.debug("LocalRxDb GetKYCDetails:", error);
      return null;
    }
  }

  async getAllKYCDetails(): Promise<KYCDetails[]> {
    try {
      const documents = await (await this.kycDetails)?.find().exec();
      return documents?.map(doc => doc.toJSON()) || [];
    } catch (error) {
      logger.debug("LocalRxDb GetAllKYCDetails:", error);
      return [];
    }
  }

  async getKYCDetailsByStatus(status: 'pending' | 'approved' | 'rejected'): Promise<KYCDetails[]> {
    try {
      const documents = await (await this.kycDetails)?.find({
        selector: { status }
      }).exec();
      return documents?.map(doc => doc.toJSON()) || [];
    } catch (error) {
      logger.debug("LocalRxDb GetKYCDetailsByStatus:", error);
      return [];
    }
  }
  ...
  async updateKYCStatus(
    ...
    try {
      ...
      logger.debug("LocalRxDb UpdateKYCStatus:", error);
      return null;
    }
  }
  ...
  async updateDocumentStatus(
    ...
    try {
      ...
      const documents = document.documents.map(doc => 
      ...
      logger.debug("LocalRxDb UpdateDocumentStatus:", error);
      return null;
    }
  }

  async deleteKYCDetails(userId: string): Promise<void> {
    try {
      const document = await (await this.kycDetails)?.findOne(userId).exec();
      if (document) {
        await document.patch({
      ...
      logger.debug("LocalRxDb DeleteKYCDetails:", error);
    }
  }
  ...
  async addKYCDocument(
    ...
    try {
      const kycDoc = await (await this.kycDetails)?.findOne(userId).exec();
      ...
      const documents = [...kycDoc.documents, newDocument];

      const updated = await kycDoc.patch({
        documents,
        updatedAt: Date.now()
      });
      return updated.toJSON();
    } catch (error) {
      logger.debug("LocalRxDb AddKYCDocument:", error);
      return null;
    }
  }
  ...
  async updateRiskLevel(
    ...
    try {
      ...
      logger.debug("LocalRxDb UpdateRiskLevel:", error);
      return null;
    }
  }
}